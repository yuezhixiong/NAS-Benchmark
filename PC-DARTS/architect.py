    ...
    for i in range(unrolled_model._layers):
      if unrolled_model.cells[i].reduction:
        if self.args.temperature[:-1] == 'Gumbel':
          ...
          n = 3
          start = 2
          beta = F.softmax(unrolled_model.betas_reduce[0:2], dim=-1)
          for i in range(unrolled_model._steps-1):
            end = start + n
            tw2 = F.softmax(unrolled_model.betas_reduce[start:end], dim=-1)
            start = end
            n += 1
            beta = torch.cat([beta,tw2],dim=0)
        ...
        if self.args.temperature[:-1] == 'Gumbel':
          ...
          n = 3
          start = 2
          beta = F.softmax(unrolled_model.betas_normal[0:2], dim=-1)
          for i in range(unrolled_model._steps-1):
            end = start + n
            tw2 = F.softmax(unrolled_model.betas_normal[start:end], dim=-1)
            start = end
            n += 1
            beta = torch.cat([beta,tw2],dim=0)

        u = compute_u(C_list[i], is_reduction=False)
      loss += torch.mul(torch.mul(alpha, u.t()).sum(dim=-1), beta).sum()
